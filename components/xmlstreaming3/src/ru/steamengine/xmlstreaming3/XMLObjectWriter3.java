...
public class XMLObjectWriter3 implements ObjectWriter, StreamInit {
    ...
    public XMLObjectWriter3() {
    }

    public XMLObjectWriter3(RegistryUser registryUser) {
    ...
    private void writeObjectProps(final Object obj,
        ...
        for (RTTIEntry entry : rtti.getProperties()) {
            ...
            switch (ident) {
                case value:
                    ...
                    if (valStored) {
                        Node valElement = new Node(XMLReadWriteConsts3.PROP_IDENT);
                        try {
                            Object val = entry.getVal(obj);
                            ...
                            if (val != null) {
                                ...
                                valElement.attribs.setItem(XMLReadWriteConsts3.PATH_IDENT, newPath);
                                valElement.attribs.setItem(XMLReadWriteConsts3.VALUE_IDENT, valValue);
                            } else {
                                valElement.attribs.setItem(XMLReadWriteConsts3.PATH_IDENT, newPath);
                            }
                ...
                case reference:
                    ...
                    if (refStored) {
                        final Node refElement = new Node(XMLReadWriteConsts3.PROP_IDENT);
                        ...
                        if (ref != null) {
                            ...
                            refElement.attribs.setItem(XMLReadWriteConsts3.PATH_IDENT, newPath);
                            refElement.attribs.setItem(XMLReadWriteConsts3.VALUE_IDENT, refValue);
                        } else {
                            refElement.attribs.setItem(XMLReadWriteConsts3.PATH_IDENT, newPath);
                        }
        ...
        for (ListSlot slot : rtti.getLists()) {
            ...
            String newPath = getPropPath(propPath, propertyName);
            Node listElements = new Node(XMLReadWriteConsts3.PROP_IDENT);
            listElements.attribs.setItem(XMLReadWriteConsts3.PATH_IDENT, newPath);
            int i = 0;
            ...
            if (st != null) {
                for (Object o : getListIterator(slot, obj)) {
                    if ((o == null) && (st instanceof PropertyRecoderEx) &&
                        ...
                        throw new WriteError("value for can not be null");

                    Node element = new Node(XMLReadWriteConsts3.ITEM_IDENT);
                    listElements.addNode(element);
                    if (o != null) {
                        //noinspection unchecked
                        byte[] bytes = st.getValue(o);
                        String valAsString = new String(bytes, UsedCharset.CHARSET);
                        element.attribs.setItem(XMLReadWriteConsts3.VALUE_IDENT, valAsString);
                    }
                    i++;
                }
    ...
    private Node writeObject(final int position,
        ...
        Node element = new Node(XMLReadWriteConsts3.OBJECT_IDENT);
        String classIdent = registryUser.getClassIdent(obj.getClass());
        element.attribs.setItem(XMLReadWriteConsts3.CLASS_IDENT, classIdent);
        element.attribs.setItem(XMLReadWriteConsts3.NAME_IDENT, getObjectName(obj, classIdent));
        if (position >= 0)
            element.attribs.setItem(XMLReadWriteConsts3.POSITION_IDENT, String.valueOf(position));
        if (rootElement == null)
            rootElement = element;

        List<Node> elements = new ArrayList<Node>();
        writeObjectProps(obj, elements, propPath); // here props are collected
    ...
    private Node writeListedObject(final Object obj,
        ...
        Node element = new Node(XMLReadWriteConsts3.ITEM_IDENT);
        List<Node> elements = new ArrayList<Node>();
        writeObjectProps(obj, elements, propPath); // props are collected here
        for (Node subNode : elements)
            element.addNode(subNode);
        return element;
    }
